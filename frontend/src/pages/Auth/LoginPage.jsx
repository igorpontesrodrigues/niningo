...
export default function LoginPage() {
  const { signInWithDiscord, signInTest } = useAuthStore();
  ...
  const handleTestLogin = async (e) => {
    ...
    setLoading(true);
    try {
      await signInTest(testName);
      // The onAuthStateChange listener in App.jsx will handle the redirect
    } catch (err) {
      console.error(err);
      alert('Erro ao fazer login de teste: ' + err.message);
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{
      ...
      >
        ...
        {/* Test Login Form */}
        <form onSubmit={handleTestLogin} style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
          <input
            type="text"
            value={testName}
            onChange={(e) => setTestName(e.target.value)}
            placeholder="Digite um usuário de teste..."
            style={{
              padding: '14px 16px',
              background: 'rgba(255,255,255,0.04)',
          ...
          <button
            ...
            style={{
              width: '100%',
              padding: '14px 24px',
              background: 'linear-gradient(135deg, #00d2c8, #0098b8)',
              border: 'none',
              borderRadius: 12,
              color: '#070912',
              fontSize: '1rem',
              fontWeight: 600,
              cursor: (loading || !testName.trim()) ? 'not-allowed' : 'pointer',
              opacity: (loading || !testName.trim()) ? 0.6 : 1,
              transition: 'all 0.2s ease',
            }}
          >
            {loading ? 'Entrando...' : 'Entrar Rápido'}
          </button>
        </form>

        {/* Discord Login Button (Optional/Secondary now) */}
        <button
          type="button"
